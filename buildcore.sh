...
cd ..
rm -r build/boot
grub-mkimage -d ./grub/i386-pc -p "(memdisk)/boot/grubfm" -c arch/legacy/grub4dos.cfg -o ./build/core.img -O i386-pc $builtin
#grub-mkimage -d ./grub/i386-pc -c arch/legacy/grub4dos.cfg -o ./build/core.img -O i386-pc-pxe -prefix="(hd0,msdos1)" newc efiemu biosdisk boot cat chain configfile cpio echo extcmd fat font gzio halt help iso9660 linux linux16 loopback ls lua lzopio memdisk minicmd newc normal ntfs ntldr part_gpt part_msdos search sleep tar test udf xzio
