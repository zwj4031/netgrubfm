...
cp arch/x64-pxe/*.* build/boot/grub/
cd build
...
cd ..
#rm -r build/boot
cp arch/legacy/MAP build/boot/grub/
cp -r arch/legacy/ntboot/* build/boot/grub/
#cp arch/legacy/wimboot build/boot/grub/
#cp arch/legacy/vbootldr build/boot/grub/
cp arch/legacy/install.gz build/boot/grub/
cd build
mkdir ../tftpboot
