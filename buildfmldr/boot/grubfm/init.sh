...
export theme_std=${prefix}/themes/StylishDark/theme.txt;
export theme=${theme_std};
search --set=efiwin -f -q /efi/boot/bootx64.efi;
search --set=efigrubfm -f -q /efi/boot/grubfmx64.efi;

menuentry "启动 windows" --class windows --class os {
	chainloader (${efiwin})/efi/boot/bootx64.efi
}                               
menuentry "启动Grub2文件管理器" --class fedora --class os {
chainloader (${efigrubfm})/efi/boot/grubfmx64.efi
}       
